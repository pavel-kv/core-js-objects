  ...
  return objects.reduce(
    (result, object) => ({
      ...
      ...Object.entries(object).reduce(
        (acc, [key, value]) => ({
          ...acc,
          [key]: result[key] === undefined ? value : result[key] + value,
        }),
        {}
      ),
    }),
...
function sellTickets(queue) {
  const result = queue.reduce((acc, value) => {
    const change = value / 25;
    if (change === 1) {
      return acc + change;
    }
    return acc - change + 1;
...
function sortCitiesArray(arr) {
  const compareStr = (_str1, _str2) => {
    const str1 = _str1.toUpperCase();
    const str2 = _str2.toUpperCase();
    if (str1 > str2) {
      return 1;
    }
    if (str1 < str2) {
  ...
  return arr
    .sort((a, b) => compareStr(a.city, b.city))
    .sort((a, b) => compareStr(a.country, b.country));
}
  ...
  return new Map(
    Object.entries(
      array.reduce((acc, object) => {
        const hash = acc;
        hash[keySelector(object)] ??= [];
        hash[keySelector(object)].push(valueSelector(object));
        return hash;
      }, {})
    )
  );
